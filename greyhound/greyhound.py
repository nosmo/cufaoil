...
class Greyhound:
    ...
    def login(self):
        ...
        post_result = self._session.post("https://app.greyhound.ie/",
                                    data={
    ...
    def parse_raw_data(self, raw_bin_data):
        ...
        for raw_entry in raw_bin_data:
            bin_data[raw_entry["waste_type"].lower()][
                raw_entry["date_time"]] = float(raw_entry["weight"])
    ...
    def get_data(self):
        ...
        result = self._session.get("https://app.greyhound.ie/collection/collection_history/green/")
        soup = BeautifulSoup(result.text, features="html.parser")

        for script in soup.find_all('script'):
            if script.contents and "blackBinsData" in script.contents[0]:
                jsdata_lines = [ i for i in script.contents[0].split("\n") if i.startswith("            res")]
                if jsdata_lines:
